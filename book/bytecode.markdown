...
Basically, they are little wrappers around the raw value. The operators will be
...
Then, we define a class that implements the base interface for each kind of expression in our
language's grammar. The simplest one is numbers:

^code number

A literal number expression evaluates to its value. Addition and
multiplication are a bit more complex because they contain subexpressions.
Before they can evaluate themselves, they need to recursively evaluate the subexpressions. Like so:
...
switched to the Bytecode pattern like this chapter describes. Look how much time I'm saving
you!
 ...
 *  *It's low-level.* Each instruction does one relatively minimal thing, and
    interesting behavior comes from *composing* the instructions.
...
This is the most complex pattern in this book, and it is not something to throw into
...
For example, see every template language ever.
...
little programs to control a robot in a language very similar to assembly language and
...
which form the backbone of the .NET platform, and it's good enough for us:
...
This is better, but it is still quite limited. Next, we need arithmetic. It's time our baby VM
...
performance and safety we want, but it has absolutely none of the designer-friendly
usability.
 ...
 *  **With a register-based VM:**
     ...
     *  *Instructions are larger.* Since instructions need arguments for stack
        ...
        The Lua develpers do not specify Lua's bytecode format, and it changes from version to
 ...
 *  **Abstraction.** If your users start defining a *lot* of stuff in data,
    ...
    In their simplest form, callable procedures aren't much more complex than a jump. The only
 ...
 *  **An untagged union:**
     ...
     *  *It's unsafe.* <span name="unsafe">This</span> is the real cost, of
        ...
        your game or cause it to crash.
 ...
 *  **If you define a text-based language:**
     ...
     *  *It will likely turn off non-technical users.* We programmers like text
        ...
        Most non-programmers don't think of text files like that. To them, text files feel
