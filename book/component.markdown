...
it without seeing some code for each of the domains that it decouples, so I'll have to sketch in a bit more of Bjørn's code than I'd like. The pattern is
really only the component *classes* themselves, but the code in them should help
 ...
 *  **By sending messages:**
     ...
     *  This is the most complex alternative. We can actually build a little
        ...
        container, it can send messages to the container, which will rebroadcast
        the message to all of its sibling components. (And back to itself for that matter;
...
that you can take for granted and that every object has -- things like position and
size.

Some domains are distinct but still closely related, such as animation and
rendering, user input and AI, and physics and collision. If you have separate
