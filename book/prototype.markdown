...
of some type. Then the one spawner class can store a function pointer:
...
a spawner creates can use it and work with pointers to `Monster`.

If we only had the `SpawnerFor<T>` class, there would be no single supertype shared by all instantiations of that template, so any code that worked with
...
Self eliminates that distinction. To look up *anything*, you just look at the
